...
export class RatingRecommendationService extends RatingsTableDictionaryOperations {
        ...
        tableData.forEach((tableRowData) => {
            tableRowData.children.forEach((rowData) => {
                if (!rowData.data.isSubTableHeader) {
                    ...
                    const rowTableData = this.updateTableSelection(
                        tableData,
                        immediateParentId,
                        identifier,
                        name,
    ...
    updateTableSelection(
        ...
        name: string,
        currency: string,
        refRowData: BlueTableRowData
    ) {
        let updatedTableRowData: BlueTableRowData;
        ...
        tableData?.forEach((tableRowData) => {
            tableRowData.children.forEach((rowData) => {
                if (
                    ...
                    updatedTableRowData = rowData;
                    updatedTableRowData.data = {
                        ...updatedTableRowData.data,
                        [ProposedRecommendationTypes.proposedWatchStatus]: refRowData.data.proposedWatchStatus,
                        [ProposedRecommendationTypes.proposedOutlook]: refRowData.data.proposedOutlook,
                        [ProposedRecommendationTypes.proposedRating]: refRowData.data.proposedRating
                    };
                    updatedTableRowData.isSelected = refRowData.isSelected;
                }
            });
        });
        return updatedTableRowData;
    }
    ...
    private manageTableDataActions([blueTableDataMap, selectedRatingRecommendationEntities, viewType]: [
        ...
        if (viewType === RatingRecommendationTableView.Debt) {
            ...
            selectedRatingRecommendationEntity?.blueTableData.forEach((tableRowData) => {
                tableRowData.children.forEach((rowData) => {
                    const immediateParentId = rowData.data.immediateParent.id;
                    const identifier = rowData.data.identifier;
                    const name = rowData.data.name;
                    const currency = rowData.data.currency;
                    this.updateTableSelection(tableData, immediateParentId, identifier, name, currency, rowData);
                });
            });
        }
